...
为了便于开箱即用，插件 v0.3.1 开始提供默认的三个虚拟模型及其回退列表：

modelscope-auto：能力较强的模型

modelscope-auto-weak：能力较弱的模型

modelscope-auto-vision：视觉多模态模型

默认配置能力排序不严谨，若有偏差，请自行修订或提 ISSUE。

由于魔搭社区 API 的不稳定性，存在不可用模型或缺少好用的模型是正常现象，自行增删即可。
